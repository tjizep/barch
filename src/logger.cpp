...
void art::raw_start_log(bool err)
{
    ...
    {   text_color = fg(fmt::color::burly_wood) | fmt::emphasis::italic;
        ...
        logged = fmt::format(header_color,"{}:E {:%d %b %Y %H:%M:%S} * BARCH ",tid, std::chrono::floor<std::chrono::milliseconds>(now));

    }else
    {
        text_color = fg(fmt::color::burly_wood);
        header_color = fg(fmt::color::white_smoke);
        logged = fmt::format(header_color,"{}:M {:%d %b %Y %H:%M:%S} * BARCH ",tid, std::chrono::floor<std::chrono::milliseconds>(now));
