...
namespace barch {
    ...
    struct sock_fun : public source {
        ...
        bool connect(const std::string& host,  const std::string& port) override {
            ...
            if (!run_to(io, s, get_rpc_connect_to_s())) {
                return false;
            }
            return !error ;
        }
        ...
        bool write(const uint8_t* data, size_t size) {
            ...
            return run_to(io, s, get_rpc_connect_to_s()) && !error;
        }
        bool write(const char* data, size_t size) override {
            async_write_to( s, asio::buffer(data, size), error);
            return run_to(io, s, get_rpc_write_to_s()) && !error;
        }
        ...
        [&](const std::error_code& result_error,std::size_t unused(result_n))
            ...
            if (!run_to(io, s, get_rpc_read_to_s())) {
                return false;
            }
            return !error;
