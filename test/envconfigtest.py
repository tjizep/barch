...
def settings(env, port, *names):
    # the child imports barch, so under a sanitizer build it needs the runtime
    # preloaded the same way this process did - see test/scale.py and TODO 207
    e = scale.barch_child_env()
    e.update(env)
...
base = settings({}, scale.port(0, default=14061), "max_scan_iterators", "max_memory_bytes", "eviction_policy")
assert base["max_scan_iterators"] != "42", "pick a different test value, 42 is the default"

# barch's own name
got = settings({"BARCH_MAX_SCAN_ITERATORS": "42"}, scale.port(1, default=14062), "max_scan_iterators")
assert got["max_scan_iterators"] == "42", got

# a redis name, and a redis byte unit with it. 256mb is binary, so 268435456
got = settings({"BARCH_MAXMEMORY": "256mb"}, scale.port(2, default=14063), "max_memory_bytes", "maxmemory")
...
got = settings({"BARCH_MAXMEMORY_POLICY": "allkeys-lru"}, scale.port(3, default=14064),
...
got = settings({"BARCH_MAXMEMORY": "100mb", "BARCH_MAX_MEMORY_BYTES": "200000000"},
               scale.port(4, default=14065), "max_memory_bytes")
...
got = settings({"BARCH_APPENDONLY": "yes", "BARCH_MAX_SCAN_ITERATORS": "37"}, scale.port(5, default=14066),
...
got = settings({"BARCH_MAX_SCAN_ITERATORS": "not-a-number"}, scale.port(6, default=14067), "max_scan_iterators")
